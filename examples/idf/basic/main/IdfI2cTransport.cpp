...
SCD41::TransferResult mappedResult(esp_err_t error,
  ...
  switch (error) {
    case ESP_OK:
      ...
      break;
    // ESP-IDF v6.0.1 synchronous transfers report NACK as INVALID_RESPONSE.
    // Generic controller/queue errors must remain failures even in the
    // driver's expected-NACK wake and attach-reconciliation phases.
